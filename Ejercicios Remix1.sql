...
-- de Mexico.
SELECT 
    cust.CompanyName, cust.Country, COUNT(*) AS Pedidos
FROM
    Customers AS cust
        JOIN
...
SELECT 
    COUNT(*) AS Pedidos, ship.CompanyName
FROM
    Orders AS ord
        JOIN
...
-- a la que pertenece.
SELECT 
    pro.ProductName, cat.Description
FROM
    Products AS pro
        JOIN
...
SELECT 
    Country, COUNT(*) AS 'EmpleadosPorPais'
FROM
    employees
GROUP BY Country;
...
SELECT 
    COUNT(*) AS 'Pedidos Clientes España'
FROM
    Customers AS cus
        JOIN
