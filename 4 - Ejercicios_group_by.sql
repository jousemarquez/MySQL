...
-- 23. Contar el número de pedidos (Orders) en lo que los años de sus campos de tipo fecha no son exactamente iguales.
-- VERSION EMILIO
SELECT COUNT(*) AS 'Pedidos_sin_años_iguales'
FROM orders 
WHERE
(year(OrderDate)!=year(ShippedDate) 
...
FROM Orders
WHERE
(YEAR(OrderDate) != YEAR(RequiredDate) AND YEAR(OrderDate) != YEAR(ShippedDate))
AND
(YEAR(RequiredDate) != YEAR(OrderDate) AND YEAR(RequiredDate) != YEAR(ShippedDate))
AND
(YEAR(ShippedDate) != YEAR(OrderDate) AND YEAR(ShippedDate) != YEAR(RequiredDate));

-- CON EXTRACT()
SELECT count(*) 'Pedidos_sin_años_iguales'
FROM Orders
WHERE
(extract(YEAR from OrderDate) != extract(YEAR from RequiredDate) AND extract(YEAR from OrderDate) != extract(YEAR from ShippedDate))
