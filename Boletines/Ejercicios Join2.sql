...
SELECT 
    ord.OrderID, emp.FirstName, emp.LastName
FROM
    Orders AS ord
        JOIN
    Employees AS emp ON ord.EmployeeID = emp.EmployeeID
WHERE
...
WHERE
    (emp.FirstName = 'Robert'
        AND emp.LastName = 'King')
        OR (emp.FirstName = 'Nancy'
        AND emp.LastName = 'Davolio')
        OR (emp.FirstName = 'Andrew'
        AND emp.LastName = 'Fuller');
-- 3.Obtener todas las ordenes por el cliente cuya compania es "Que del?cia"
SELECT 
    ord.OrderID, ord.CustomerID, cust.CompanyName
FROM
    Orders AS ord
        JOIN
    Customers AS cust ON ord.CustomerID = cust.CustomerID
WHERE
    cust.CompanyName LIKE 'Que del?cia';
...
FROM
    OrderDetails AS ord
        JOIN
    Products AS pro ON ord.ProductID = pro.ProductID
WHERE
    ord.OrderID = '10257';
...
FROM
    Orders AS ord
        JOIN
    OrderDetails AS ordd ON ord.OrderID = ordd.OrderID
        JOIN
    Products AS pro ON ordd.ProductID = pro.ProductID
WHERE
    ord.OrderDate BETWEEN '1997-01-01' AND CURDATE();
-- 6.Mostar el ID del pedido, fecha del pedido y el total de productos de cada pedido
SELECT 
    ordd.OrderID,
    ord.OrderDate,
    SUM(ordd.Quantity) 'Total de productos'
FROM
    Orders AS ord
        JOIN
    OrderDetails AS ordd ON ord.OrderID = ordd.OrderID
...
SELECT 
    cat.CategoryID, cat.CategoryName, SUM(pro.UnitsInStock)
FROM
    products AS pro
        JOIN
    categories AS cat ON pro.CategoryID = cat.CategoryID
WHERE
    cat.CategoryID IN (3 , 5, 8)
...
FROM
    orderdetails AS ordd
        JOIN
    products AS prod ON ordd.ProductID = prod.ProductID
        JOIN
    suppliers AS sup ON sup.SupplierID = prod.SupplierID
ORDER BY OrderID;
-- 11. Por cada pedido, obtener el ID de pedido, ID de empleado y el nombre de empleado.
SELECT 
    ord.OrderID, ord.EmployeesID, emp.FirstName
FROM
    Orders AS ord
        JOIN
    Employees AS emp ON ord.EmployeeID = emp.EmployeeID;
...
SELECT 
    pro.ProductID, pro.CategoryID, cat.CategoryName
FROM
    Products AS pro
        JOIN
    Categories AS cat ON pro.CategoryID = cat.CategoryID
...
SELECT 
    emp.EmployeeID,
    CONCAT(emp.FirstName, ' ', emp.LastName) 'Empleado',
    empterr.TerritoryID
FROM
    Employees AS emp
        JOIN
    EmployeeTerritories AS empterr ON emp.EmployeeID = empterr.EmployeeID;
-- 14. Por cada cliente, mostrar el ID de cliente, nombre de contacto y pedidos.
SELECT 
    cust.CustomerID, cust.ContactName, ord.OrderID
FROM
    Customers AS cust
        JOIN
...
SELECT 
    cust.CustomerID, cust.ContactName, COUNT(*) 'Tº Pedidos'
FROM
    Customers AS cust
        JOIN
    Orders AS ord ON cust.CustomerID = ord.CustomerID
...
SELECT 
    ord.OrderID,
    cust.Address 'Dirección del empleado',
    emp.Address 'Dirección del empleado'
FROM
    orders AS ord
        JOIN
    employees AS emp ON ord.EmployeeID = emp.EmployeeID
...
SELECT 
    pro.ProductID,
    pro.ProductName,
    supp.Address 'Dirección del proveedor'
FROM
    Products AS pro
        JOIN
    Suppliers AS supp ON pro.SupplierID = supp.SupplierID
ORDER BY pro.ProductID ASC;
...
FROM
    Products AS pro
        JOIN
    Categories AS cat ON cat.CategoryID = pro.CategoryID;
...
SELECT 
    ...
    ordd.UnitPrice,
    emp.EmployeeID,
    CONCAT(emp.FirstName, ' ', emp.LastName) 'Nombre de empleado'
FROM
    OrderDetails AS ordd
        JOIN
...
SELECT 
    cat.CategoryName, SUM(pro.UnitsInStock) 'Total de productos'
FROM
    Products AS pro
        JOIN
    categories AS cat ON pro.CategoryID = cat.CategoryID
...
FROM
    Customers AS cust
        JOIN
    Orders AS ord ON cust.CustomersID = ord.Customerid
        JOIN
    Suppliers AS supp ON supp.SupplierID = pro.SupplierID
        JOIN
    Employees AS emp ON emp.EmployeeID = ord.EmployeeID
        JOIN
    Products AS pro ON pro.ProductID = ordd.ProductID
WHERE
    OrderID LIKE 10794;
...
SELECT 
    cust.CompanyName 'Compañia cliente',
    ord.OrderID,
    ord.OrderDate 'Fecha orden',
    orddet.ProductID,
    orddet.Quantity 'Cantidad',
    prod.ProductName,
    sup.CompanyName 'Proveedor',
    sup.City 'Ciudad provee'
FROM
    orders AS ord
        JOIN
    orderdetails AS orddet ON (ord.OrderID = orddet.OrderID)
        JOIN
    products AS prod ON (orddet.ProductID = prod.ProductID)
        JOIN
    customers AS cust ON (ord.CustomerID = cust.CustomerID)
        JOIN
    suppliers AS sup ON (prod.SupplierID = sup.SupplierID);
...
SELECT 
    COUNT(*) AS Proveedores, Country
FROM
    Suppliers
WHERE
    LOWER(SUBSTRING(ContactName, 1, 1)) BETWEEN 'e' AND 'p%'
GROUP BY Country
...
SELECT 
    Boss.EmployeeID,
    CONCAT(Boss.FirstName, ' ', Boss.LastName) 'Boss',
    COUNT(*) 'Empleados a cargo'
FROM
    employees AS Boss,
    employees AS Slave
WHERE
    Slave.ReportsTo = Boss.EmployeeID
...
FROM
    Employees AS emp
        JOIN
    Orders AS ord ON ord.EmployeeID = emp.EmployeeID
...
SELECT 
    cust.CompanyName AS 'Nombre de cliente',
    COUNT(*) AS Pedidos,
    SUM(ordd.UnitPrice * ordd.Quantity) AS Ventas
FROM
    Customers AS cust
        JOIN
    OrderDetails AS ordd ON (ord.OrderID = ordd.OrderID)
...
SELECT 
    cust.CompanyName AS 'Nombre de cliente', COUNT(*) AS Pedidos
FROM
    Orders AS ord
        JOIN
    Customer AS cust ON (ord.CustomerID = cust.CustomerID)
